...
// 예제2.
// 메인 화면 : 나의 기록 리스트. TabNavigator로 구현
// Screen1 : 
//  - 위치(위도, 경도)를 받아오는 백그라운드 스레드 실행. 
//  - 촬영 화면. (촬영 기능, 사진 불러오기 기능(Optional))
//  - 촬영 후 Screen2로 이동
// Screen2: 
// - Image: 촬영한 사진을 보여줌. 
// - 아래에는 사용자가 기록할 수 있는 텍스트 박스가 있음
// - 그 아래에는 '등록' 버튼이 있음
// - 등록 버튼을 누르면, 사용자가 입력한 텍스트와 촬영한 사진을 로컬에 저장함.
// 버튼1. 카메라 화면으로 이동
// 버튼2. 위치 화면으로 이동
// Local storage directory for photos and records
const RECORDS_DIRECTORY = FileSystem.documentDirectory + 'records/';
const RECORDS_FILE = FileSystem.documentDirectory + 'records.json';
